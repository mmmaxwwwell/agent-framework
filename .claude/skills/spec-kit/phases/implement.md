...
**Zero-skips (MANDATORY):** Any test reported as skipped is treated as a FAIL. `skip > 0` = FAIL, regardless of reason. If a test didn't run, it's a skip — missing database, unreachable service, missing credential, `describe.skip`, `t.Skip`, `pytest.mark.skipif`, `canRun` gates, try/catch that swallows setup errors, all equivalent. The test-run command MUST exit non-zero when any skip is recorded. Do NOT patch a failing skip by adding more guards — remove the guard and make the test run. If a test physically cannot run in this environment, it should not be in the test list for this environment (use separate files / separate npm scripts / CI job matrices), not skipped at runtime. See `reference/testing.md` § "Zero-skips rule" for the full pattern and `reference/cicd.md` § "Skip-as-failure CI validation" for the CI-level enforcement.

**Integration tests exercise the whole app (MANDATORY):** Integration tests MUST drive the code against real services — the real database, the real auth server, the real message bus. No service stubs inside integration test files. No conditional skip on service availability — if the service isn't up, start it or fail loudly. Skip guards like `if (!DATABASE_URL) describe.skip(...)` or `async beforeAll() { if (!await svcUp()) return; }` are banned.

**Test tier placement (MANDATORY):** Every test goes in exactly one of three tiers — unit, integration, or E2E — based on what it drives and what infrastructure it needs. Integration tests NEVER touch the mobile emulator / simulator / browser — if a test drives UI, it's E2E. E2E tests NEVER substitute for integration tests — write the integration test too, because the pre-E2E gate requires the integration tier to be green before the emulator even boots. Unit tests NEVER substitute for integration tests — mocking the DB hides cross-boundary bugs. File-name convention is load-bearing: `*.integration.test.ts` runs against live services in the pre-E2E gate, `*.test.ts` runs in implement-phase validation, `*.e2e.test.ts` runs in the MCP crawl. See `reference/testing.md` § "Test tier taxonomy" for the full table.

**Test depth (MANDATORY):** No vacuous assertions (`expect(x).toBeDefined()` alone, `expect(x).toBeTruthy()` alone, bare `typeof` checks). Every new endpoint, handler, or public function needs BOTH a happy-path test AND at least one error-path test. No mocking inside tier boundaries (integration tests don't mock the DB; unit tests don't mock internal modules). Prefer observable-behavior assertions over "was function X called" spies. The phase-validation code review enforces this — violations block phase completion.
