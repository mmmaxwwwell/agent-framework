...
1. Read ALL files in the referenced \`$SPEC_DIR/validate/<phase>/\` directory — these contain the failure history from previous attempts including test output, assertion details, stack traces, and what was tried before.
2. Read \`test-logs/\` for the latest structured failure output (summary.json + failures/).
3. Diagnose the root cause from the accumulated evidence. **Tests are the spec** — fix the code, not the tests. Exception: if a test is genuinely wrong (tests behavior the spec doesn't require), fix the test with a comment explaining why.
4. Fix the code, then proceed to Step 4 (phase validation) — run the validation commands again. If it still fails, another phase-fix task will be appended.
...
## Step 4: Phase validation (only at phase boundaries)

Check whether this task is the **last unchecked task in its phase** (all other tasks in the phase are \`- [x]\` or \`- [~]\`). If it is NOT the last task in the phase, skip to Step 5.
...
- For early phases (e.g., Phase 1 Setup), the build/test commands and test-logs infrastructure may not exist yet — that's fine, verify what you can

### If validation passes

The phase checkpoint is met. Continue to Step 5.

### If validation fails — write state and append a phase-fix task

Do NOT attempt to fix failures yourself. Instead, capture the failure state and let the next agent iteration handle it with fresh context:

1. **Determine the phase name** from the task file heading (e.g., \`phase1\`, \`phase2\`, \`phase3\`, etc. — use the phase number or a short kebab-case slug).

2. **Create a validation run file** at \`$SPEC_DIR/validate/<phase>/<N>.md\` (where N increments: \`1.md\`, \`2.md\`, \`3.md\`, ...):
   \`\`\`markdown
   # Phase validation failure: <phase> (attempt <N>)
   ...
   <if test-logs/ exists, paste the contents of summary.json here>

   ## Key failures
   <for each failure in test-logs/<type>/<timestamp>/failures/, paste the full failure log>
   <if no test-logs/ infrastructure yet, paste the relevant stderr/stdout — the most informative ~100 lines>

   ## Tasks completed in this phase
   <list all tasks in this phase and what they implemented>
   \`\`\`

3. **Check the attempt count** — look at how many files exist in \`$SPEC_DIR/validate/<phase>/\`:
   - **If fewer than 10 attempts**: append a fix task to \`$TASK_FILE\` at the end of the current phase (before the next phase heading):
     \`\`\`
     - [ ] <phase>-fix<N> Fix phase validation failure: read \`$SPEC_DIR/validate/<phase>/\` for failure history, read \`test-logs/\` for latest failures, fix the code (tests are the spec), then re-run validation
     \`\`\`
   - **If 10 attempts already**: the fix loop is exhausted. Write \`BLOCKED.md\` with the failure history and stop.

This way each fix attempt gets a fresh agent with full context, and the failure history accumulates on disk so nothing is lost between runs.
...
INIT
  ...
  # Ensure validate directory exists for the fix-validate loop
  mkdir -p "$SPEC_DIR/validate"
