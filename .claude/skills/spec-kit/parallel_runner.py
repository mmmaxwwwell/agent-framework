...
# Stale-output watchdog: kill a sub-agent if its log file hasn't been written
# to for this many seconds.  Healthy agents continuously stream JSON events;
# a gap this long means the API connection is dead or the agent is stuck.
CI_AGENT_STALE_OUTPUT_S = 5 * 60
...
class Runner:
    ...
    def __run_ci_loop_inner(self, task: Task, spec_dir: str, task_file: Path,
        ...
        def _wait_for_subagent(sub_task: Task, prompt: str, label: str,
            ...
            # Watchdog: poll for completion while checking log for signs of life
            stale_killed = False
            while proc.poll() is None:
                time.sleep(10)
                if log_p.exists():
                    idle_s = time.time() - log_p.stat().st_mtime
                    if idle_s > CI_AGENT_STALE_OUTPUT_S:
                        ci_log(
                            f"{label} (Agent {aid}, pid {proc.pid}) "
                            f"stale for {int(idle_s)}s — killing"
                        )
                        proc.kill()
                        proc.wait()
                        stale_killed = True
    ...
    def _run_e2e_loop_inner(self, task: Task, spec_dir: str, task_file: Path,
        ...
        # Stale-output watchdog: kill an agent if its log file hasn't been
        # written to for this many seconds.  Healthy agents continuously stream
        # JSON events to their log; a gap this long means the API connection
        # is silently dead or the agent is otherwise stuck.
        AGENT_STALE_OUTPUT_S = 5 * 60
        ...
        def _wait_for_subagent(sub_task: Task, prompt: str, label: str,
            ...
            # Watchdog: poll for completion while checking the log file for
            # signs of life.  If the log hasn't been written to in
            # AGENT_STALE_OUTPUT_S seconds, the API connection is likely dead.
            stale_killed = False
            while proc.poll() is None:
                time.sleep(10)
                if log_p.exists():
                    idle_s = time.time() - log_p.stat().st_mtime
                    if idle_s > AGENT_STALE_OUTPUT_S:
                        e2e_log(
                            f"{label} (Agent {aid}, pid {proc.pid}) "
                            f"stale for {int(idle_s)}s — killing"
                        )
